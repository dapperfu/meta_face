...
### Sharing sidecars with meta_pose

The same `photo.scar` file can hold both `face.*` and `pose.*` keys. Writes use `SidecarDocument.update_path` (sidecar-rs ≥ 0.2.1) with a per-file lock so `mf` and `mp` workers can run concurrently without dropping each other's data.
