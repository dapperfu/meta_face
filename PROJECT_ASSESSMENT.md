...
The original [pipeline plan](/projects/spring_photography/meta_face/plans/meta-face-pipeline.md:3) specified SCRFD detection, ArcFace embeddings, HDBSCAN clustering, GPU-host execution, and a runnable CLI/worker structure. Subsequent plans broadened this to dlib, Detectron2, annotation notebooks, collection statistics, and 17 optional facial-analysis adapters.
...
|---|---|---|
| Detect faces and compute embeddings | SCRFD/ArcFace and dlib pipelines are implemented. Existing sidecars demonstrate both ran. Detectron2 is wired in, but its default model detects people rather than faces. | A repeatable run on representative photos, verified face boxes, correct embedding dimensions, and consistent face-to-embedding alignment. |
...
2. **Default Detectron2 results have the wrong meaning.** The configuration selects COCO RetinaNet and class `0`, explicitly identified in the code as `person`. The backend stores these object boxes directly as `face.detectron2.faces` and `face_count`. This does not fulfill the planned face-detector role. A face-trained model or a separate person-detection namespace is needed before these records can be treated as face detections. See [model selection](/projects/spring_photography/meta_face/src/meta_face/config.py:123) and [payload construction](/projects/spring_photography/meta_face/src/meta_face/backends/detectron2_backend.py:99).
...
The original plans say ArcFace-only embeddings and an InsightFace-only default, while current defaults include dlib and Detectron2. The Detectron2 plan describes a face-trained WIDER FACE checkpoint, while the current default is COCO. These plans are useful history but are not a consistent current specification.
